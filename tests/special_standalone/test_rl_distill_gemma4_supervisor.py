...
def test_numerical_anomaly_reason_accepts_large_finite_gradient_when_ceiling_disabled():
    assert (
        supervisor.numerical_anomaly_reason(
            {"train/loss": 0.156, "train/grad_norm": 7230.0},
            max_grad_norm=0.0,
        )
        is None
    )
...
def test_child_environment_pins_verified_gemma4_batching_contract(tmp_path: Path):
    ...
    assert "VERL_MAX_PRECLIP_GRAD_NORM" not in environment
