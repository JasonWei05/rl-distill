...
VAL=/tmp/gemma4_e4b_val32            # reference traces (read-only here): the plot loop mirrors S3 into $VAL/students with --delete
OUTROOT=${OUTROOT:-/tmp/gemma4_stk50_passk}  # our outputs live OUTSIDE $VAL: the hourly sync --delete removed our .partial traces twice
...
run_eval() { local model=$1 tag=$2 sha=$3; local out=$OUTROOT/$tag; mkdir -p "$out/traces"
...
[ -f $OUTROOT/base_e2b__x32_medium/metrics.json ] || run_eval "$E2B" base_e2b__x32_medium bde9e800223cdd62228ce39e0305398f6ada05b98adaf438b0b3d3d3c3015561
echo "$(date -u +%FT%TZ) PLOT"
.venv/bin/python $S/plot_passk_from_traces.py   # matplotlib lives in .venv, not .venv-gemma4 --out $S/figures/passk_12b_onpolicy_stk50_vs_teacher.png \
  ...
  --trace "E2B base x32=$OUTROOT/base_e2b__x32_medium/traces/base_e2b__x32_medium__id_medium.jsonl" \
  ...
  --trace "12B on-policy (student top-128) step 50=$OUTROOT/onpolicy_studenttop128_12b__step_000050/traces/onpolicy_studenttop128_12b__step_000050__id_medium.jsonl"
