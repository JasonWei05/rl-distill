...
# B200 single-node defaults for DAPO RL with Gemma 3 4B PT. This mirrors the
# current 12B B200 one-node run defaults, except for the model and optimizer
# defaults requested for the 4B run.
...
HF_PUSH_REPO=${HF_PUSH_REPO:-"JWei05/DAPO-Gemma3-4B-PT-RL-DAPO17k"}
...
if [ -z "${MODEL_PATH}" ]; then
    if [ "${PREPARE_MODEL}" = "1" ] && [ -n "${MODEL_SUBFOLDER}" ]; then
