#!/usr/bin/env bash
# On-policy distillation: Gemma3-4B-IT student <- trained DAPO-Gemma3-27B teacher.
#
# Each "step":
#   1. Student samples 1 response per prompt for 128 prompts (vLLM rollout).
#   2. Teacher (27B) computes its log_prob of *each student-sampled token*
#      via colocated vLLM (TP=2) — no top-k, just the sampled token.
#   3. Loss = single-sample reverse-KL estimator on those token logprobs
#      (loss_mode=k3): student_lp - teacher_lp + correction → ≥ 0, low variance.
#
# Schedule: 200 steps × bs 128 = 25,600 prompt samples ≈ 1.47 epochs over 17,398 DAPO prompts.
# Optimizer: lr=1e-5, lr_warmup_steps=20, lr_scheduler_type=constant.
#
# IMPORTANT: requires the dapo trainer's fit() to call _compute_teacher_colocate()
#   after rollout. See rl-distill-scripts/DISTILLATION.md ("planned, ~5 lines").
set -xeuo pipefail
...
# ---- Snapshot trained 27B teacher locally if not already present ----
# Default to /tmp because /home/tiger rootfs is usually too full for a 54GB model.
TEACHER_REV="${TEACHER_REV:-step_000040}"
TEACHER_PATH="${TEACHER_PATH:-/tmp/verl/models/dapo-gemma3-27b-it-${TEACHER_REV}}"
...
exp_name=${EXP_NAME:-"onpolicy-gemma3-4b-$(date +%Y%m%d-%H%M)"}
...
export HF_PUSH_REPO="${HF_PUSH_REPO:-JWei05/gemma3-4b-it-on-policy-distilled-from-dapo27b}"
export HF_PUSH_PRIVATE="${HF_PUSH_PRIVATE:-false}"
export HF_PUSH_MAX_TO_KEEP="${HF_PUSH_MAX_TO_KEEP:-10}"

# ----- Distillation hyperparams -----
# 128 prompts per RL step × 1 student response per prompt = 128 sequences/step.
# 200 steps × 128 = 25,600 / 17,398 prompts ≈ 1.47 epochs.
train_prompt_bsz=128
gen_prompt_bsz=${train_prompt_bsz}
n_resp_per_prompt=1
train_prompt_mini_bsz=128       # one mini-batch per step → on-policy update

max_prompt_length=$((1024 * 2))
max_response_length=$((1024 * 20))

# Pure distillation: no overlong reward shaping, no KL-to-ref, no clipping issues.
...
# Student rollout sampling — T=1.0, top_p=1.0 to match gemma3_4b_it_fsdp2_20k.sh.
# With T=1.0, the teacher's prompt_logprobs is also computed at T=1.0 (same distribution),
# giving a clean reverse-KL on-policy distillation objective.
temperature=1.0
top_p=1.0
top_k=-1
val_top_p=0.7

# Teacher inference: TP=2 vLLM colocated on the same node as student.
...
python3 -m dapo.main_dapo \
    ...
    ++distillation.teacher_model.inference.gpu_memory_utilization=0.35 \
