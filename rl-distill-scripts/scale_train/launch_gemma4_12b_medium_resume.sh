...
GPUS_PER_INSTANCE="${GPUS_PER_INSTANCE:-8}"   # 4 = resume from the world_size-4 reshard (CKPT_SUFFIX=-local4 prefixes), 8 = original shards
CKPT_SUFFIX="${CKPT_SUFFIX:-}"                # "" -> original 8-rank prefixes (…/12b-medium); "-local4" -> the 4-rank reshard prefixes
...
ENV_VARS+=",ENABLE_OVERLONG_BUFFER=True,GEN_PROMPT_BSZ=64,GEN_TP=1,LOG_TRAIN_GENERATIONS=100,LOG_VAL_GENERATIONS=100,MAX_ACTOR_CKPT_TO_KEEP=${MAX_ACTOR_CKPT_TO_KEEP:-6},MAX_MODEL_LEN=12288,MAX_PROMPT_LENGTH=4096,MAX_RESPONSE_LENGTH=8192"
...
ENV_VARS+=",ROLLING_CHECKPOINT_ENABLED=True,ROLLING_CHECKPOINT_FREQ=${ROLLING_CHECKPOINT_FREQ:-5},FULL_CHECKPOINT_S3_URI=${S3_BASE}-full-checkpoints/12b-medium${CKPT_SUFFIX},RUN_ARTIFACT_S3_URI=${S3_BASE}/gemma4-12b-medium${CKPT_SUFFIX}"
ENV_VARS+=",WANDB_RUN_ID=g4ds26b-12b-medium-s42-v1,WANDB_RESUME=allow"
echo "LAUNCH_ENV ${ENV_VARS}"
exec bash launch_st_with_code.sh --gpus-per-instance "${GPUS_PER_INSTANCE}" --priority high --allow-borrowing --active-deadline-hours 240 \
  --run-file run_gemma4_pt_deepscaler_4of4strict_rl.sh --job-name "${JOB_NAME:-g4-12b-med-resume${CKPT_SUFFIX}}" --env-vars "${ENV_VARS}" "$@"
