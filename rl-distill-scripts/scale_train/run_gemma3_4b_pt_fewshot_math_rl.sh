#!/usr/bin/env bash
# ScaleTrain entrypoint: Gemma 3 4B PT few-shot math DAPO RL (single node, full 8-GPU H100 node).
# Same config as the local 1B runs (unified few-shot prompt, DAPO-val-x16-only pass@k/maj@k, 20k
# response + 4k overlong buffer, val=train sampling, SAVE_FREQ=25 + HF push /25, wandb) via the
# shared core gemma3_pt_fewshot_math_rl.sh. Isolated local Ray (address=local), all 8 GPUs.
#
# Launch:
...
# model (gated; HF_TOKEN from .env) + data (from HF, idempotent) — builds dapo_rl_val100_x16 etc.
...
HF_PUSH_REPO="${HF_PUSH_REPO:-JWei05/DAPO-Gemma3-4B-PT-FewShotMath}" \
...
DATA_DIR="${DATA_DIR}" CKPTS_DIR="${RAY_DATA_HOME}/ckpts/gemma3-4b-pt-fewshot-math" \
    bash rl-distill-scripts/gemma3_pt_fewshot_math_rl.sh \
      +ray_kwargs.ray_init._temp_dir=/tmp/ray_4b_fewshot \
