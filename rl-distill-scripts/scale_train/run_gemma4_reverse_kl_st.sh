...
#   --env-vars "MODELS=distilled,base"   (default: both)   STUDENT_REPO/REVISION/SUBFOLDER pin the distilled export.
set -euo pipefail
...
STUDENT_REPO="${STUDENT_REPO:-JWei05/Distill-gemma4-e4b-base-medium-to-12b-base}"; STUDENT_REVISION="${STUDENT_REVISION:-92368d1f020e685436113d93dc1f75c64033570f}"; STUDENT_SUBFOLDER="${STUDENT_SUBFOLDER:-step_001000}"
BASE_12B_REV=023679ed352de9bb66cc873c9009ce3482585c08; TEACHER_REV=411aa17b749aa952df1359d2dcea73917a544d9a
...
for m in ${MODELS//,/ }; do
  case "$m" in
    distilled)
      STUDENT_PATHS[distilled]="${WORK}/models/distilled_12b_${STUDENT_SUBFOLDER}"
      python rl-distill-scripts/data/download_hf_subfolder.py --repo-id "${STUDENT_REPO}" --revision "${STUDENT_REVISION}" --subfolder "${STUDENT_SUBFOLDER}" \
        --output-dir "${STUDENT_PATHS[distilled]}" --overwrite --metadata-repo google/gemma-4-12B --metadata-revision "${BASE_12B_REV}" ;;
    base)
      STUDENT_PATHS[base]="$(python -c "from huggingface_hub import snapshot_download; print(snapshot_download('google/gemma-4-12B', revision='${BASE_12B_REV}'))")" ;;
    *) echo "FATAL: unknown model tag ${m}" >&2; exit 2 ;;
  esac
done
for m in ${MODELS//,/ }; do
  TAG="12b_${m}__vs_e4b_base__medium_q${QUESTIONS}_s${SAMPLES}_top${TOPK}"; OUT="${WORK}/results/${TAG}"; mkdir -p "${OUT}"
