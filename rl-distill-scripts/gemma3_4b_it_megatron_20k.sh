...
project_name='DAPO'
exp_name=${EXP_NAME:-"DAPO-Gemma3-4B-IT-Megatron-$(date +%Y%m%d-%H%M)"}
...
# Ray. Launch this on the Ray head of a 2-node H100 cluster, or override
# RAY_ADDRESS to a concrete address if the cluster was started differently.
...
HF_PUSH_REPO=${HF_PUSH_REPO:-"JWei05/dapo-gemma3-4b-it-megatron"}
TRAIN_FILE=${TRAIN_FILE:-"${RAY_DATA_HOME}/data/dapo_openmath2_mix.parquet"}
...
if [ ! -f "${MODEL_PATH}/config.json" ]; then
    ...
    python3 rl-distill-scripts/setup_model.py --size 4b --variant it --output-dir "${MODEL_PATH}"
fi
...
# Megatron parallelism. Defaults keep the dense 4B actor data-parallel across
# all 16 GPUs in a 2-node H100 cluster.
ACTOR_TP=${ACTOR_TP:-1}
ACTOR_PP=${ACTOR_PP:-1}
ACTOR_CP=${ACTOR_CP:-1}
ACTOR_EP=${ACTOR_EP:-1}
...
python3 -m dapo.main_dapo \
    ...
    actor_rollout_ref.actor.clip_ratio_c=4.0 \
