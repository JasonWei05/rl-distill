...
MODEL_PATH=${MODEL_PATH:-"${RAY_DATA_HOME}/models/gemma-3-4b-pt"}
CKPTS_DIR=${CKPTS_DIR:-"/tmp/verl/ckpts/${project_name}/${exp_name}"}
HF_PUSH_REPO=${HF_PUSH_REPO:-"JWei05/dapo-gemma3-4b-pt-megatron"}
...
# Megatron parallelism (Gemma3-4B on 8xB200, single node)
ACTOR_TP=${ACTOR_TP:-1}
ACTOR_PP=${ACTOR_PP:-1}
ACTOR_CP=${ACTOR_CP:-1}
ACTOR_EP=${ACTOR_EP:-1}
...
# Force ray to use 127.0.0.1 instead of the routable podIP so we don't collide
# with the system FSDP2 ray on shared default ports (dashboard_agent 52365, etc).
# This flag flips ray._private.services.get_node_ip_address() to return loopback.
export RAY_ENABLE_WINDOWS_OR_OSX_CLUSTER=false
