...
project_name='DAPO'
exp_name=${EXP_NAME:-"DAPO-Gemma3-27B-IT-$(date +%Y%m%d-%H%M)"}
...
# Ray — 4-node setup, point RAY_ADDRESS at the head node
...
MODEL_PATH=${MODEL_PATH:-"${RAY_DATA_HOME}/models/gemma-3-27b-it"}
# Writing checkpoints to NVMe (/tmp, ~918 GB free) — the rootfs only has ~18 GB.
...
# Performance — 27B needs offload + TP=2 for vLLM to fit with 20k ctx on B200
sp_size=1
