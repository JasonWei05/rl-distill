...
#   TRAINING_ENGINE_AUDIT_RECEIPT       - passing real FSDP2 train/backward audit for an overlay
...
for positive_name in FULL_VOCAB_KL_CHUNK_SIZE TRAIN_BATCH_SIZE MICRO_BATCH_SIZE_PER_GPU MAX_LENGTH \
...
for boolean_name in CLAMP_MIN_TOPK_KL CHECKPOINT_DISTILL_CHUNKS; do
...
# Keep stored/master parameters and optimizer state in FP32, but expose BF16
# parameter views during FSDP forwards. The target overlay was generated by a
# BF16 HF forward; FP32 FSDP views leave embedding and other non-autocast work
# in FP32 and measurably change the target distribution.
...
case "${FSDP_PARAM_DTYPE}" in
    ...
    fp32|fp16)
        if [ "${ALLOW_UNSAFE_GEMMA4_FSDP_PARAM_DTYPE}" != "true" ]; then
            echo "Gemma 4 production requires FSDP_PARAM_DTYPE=bf16; set ALLOW_UNSAFE_GEMMA4_FSDP_PARAM_DTYPE=true only for a deliberate diagnostic" >&2
            exit 2
        fi
        ;;
...
if [ "${DATASET_SCHEMA_VERSION:-}" = "gemma4-hf-bf16-sdpa-topk-overlay-v1" ]; then
    ...
    "${PYTHON_BIN}" "${PROJECT_ROOT}/rl-distill-scripts/data/verify_gemma4_fsdp2_training_audit.py" \
        ...
        --expected-train-batch-size "${TRAIN_BATCH_SIZE}" \
