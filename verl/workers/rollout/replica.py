...
class RolloutReplica(ABC):
    ...
    async def release_cached_memory(self):
        """rl-distill fork: ask each rollout server to empty its CUDA caching allocator."""
        await asyncio.gather(*[server.release_cached_memory.remote() for server in self.servers])
