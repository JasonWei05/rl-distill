...
# megatron.core._rank_utils and megatron.training only exist in newer/full
# Megatron-LM installs; fall back to env-based helpers on megatron-core pip
# installs (e.g. core_v0.16.0).
try:
    ...
    def get_rank_safe() -> int:
        """Distributed rank, whether or not torch.distributed is initialized."""
        if torch.distributed.is_available() and torch.distributed.is_initialized():
            return torch.distributed.get_rank()
        return int(os.getenv("RANK", "0"))

    def get_world_size_safe() -> int:
        """Distributed world size, whether or not torch.distributed is initialized."""
        if torch.distributed.is_available() and torch.distributed.is_initialized():
...
try:
    ...
    def get_local_rank_preinit() -> int:
        """Local rank from the environment, before process-group init."""
